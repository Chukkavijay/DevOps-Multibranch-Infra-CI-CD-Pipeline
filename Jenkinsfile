...
pipeline {
    agent none
    environment {
        PROJECT = "WELCOME TO DEVOPS - JENKINS INFRASTRUCTURE CI/CD TEST"
    }
    stages {
     stage('For Parallel Stages') {
      parallel {
        stage('Deploy To Development') {
            agent { label 'DEV' }
            environment {
            DEVDEFAULTAMI = "ami-0520fe6654f7fb7ba"
            ...
            stages {
                stage('Perform Packer Build') {
                    ...
                    steps { //Assume Role in MasterAccount AWS Account
                        withAWS(role:'DevOpsJenkinsAssumeRole', roleAccount:'053490018989', duration: 900, roleSessionName: 'jenkins-session') {
                            sh 'pwd'
                            sh 'ls -al'
                ...
                stage('Validate Ansible Playbook & Dry Run') {
                    ...
                    steps {
                        ...
                        withCredentials([file(credentialsId: 'AWSKEY', variable: 'ansiblepvtkey')]) {
                        sh "sudo cp \$ansiblepvtkey $WORKSPACE"
                        sh "ls -al"
                        sh "sudo ansible-playbook -i invfile docker-swarm.yml -u ansibleadmin --private-key=AWSKEY.pem --check"
                        }
        ...
        stage('Deploy To Production') {
            agent { label 'PROD' }
            environment {
            PRODEFAULTAMI = "ami-030c01426c078ebc6"
            PACKER_ACTION = "NO" //YES or NO
            TERRAFORM_APPLY = "NO" //YES or NO
            ...
            stages {
                stage('Perform Packer Build') {
                    ...
                    steps { //Assume Role in SECOUND AWS Account
                        withAWS(role:'DevOpsJenkinsAssumeRole', roleAccount:'009412611595', duration: 900, roleSessionName: 'jenkins-session') {
